...
On these services the audio does not travel in a shape the traffic watcher recognises, so an album page used to list nothing at all. Play a track and it appears by name, because the page tells the browser what is playing, which is the same thing that fills the media keys on your keyboard. That gives the real track and artist to search for, instead of the album name sitting in the page title. It reports whatever is playing, so if an advert is on you will see the advert named, and can wait for the track.
